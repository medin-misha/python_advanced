...
def get_pids(port: int) -> List[int]:
    ...
    """
    command:list = f"lsof -i :{port}".split()
    pids:list[str] = []
    try:
        result_command = subprocess.check_output(command).decode()
        pids.append(result_command.split()[10])

    except subprocess.CalledProcessError:
        print(f"port {port} is unoccupied")
...
def free_port(port: int) -> None:
    ...
    kill_process:list[str] = ["kill", "-9",""]
    for port in pids:
        kill_process[2] = str(port)
        subprocess.check_output(kill_process)
